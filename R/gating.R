...
gating <- function(dat,
                   ...
                   numerator = TRUE,
                   alpha = 0.05,
                   p_cor = c("none", "correlated", "uncorrelated"),
                   nbc = NULL,
                   doplot = FALSE,
  ...
  if (n_condition == 2 & nrow(unique(dat[ , 3])) != 2) { stop("Must have two conditions") }
  ...
  if (k == 1) { j <- 1 } else { j <- k * 2 - 1 } # vars indexing per gate
    ...
    if (k == 1) { p_cor <- p_cor } else { p_cor <- "none"}
    ...
    pols <- maptools::unionSpatialPolygons(out_pol[out_pol$layer == v, ],
                                           IDs = rep(1, length(out_pol[out_pol$layer == v, ])))
    ...
    if (k == n_gate) {
      cat(paste("Observations within significant", type_cluster, "cluster(s) of Gate", k, "\n",sep = " "))
      output <- dat[which(dat[ , 1] %in% dat_gate[ , 1]), ]
      out_list <- list("obs" = output, "gate" = list_gate)
      return(out_list)
    }
